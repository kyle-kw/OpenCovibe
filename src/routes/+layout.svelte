<script lang="ts">
  ...
  async function refreshWindowMaximized() {
    try {
      ...
      dbgWarn("layout", "window maximize state unavailable", e);
    }
  }
  ...
  async function minimizeWindow() {
    try {
      ...
      dbgWarn("layout", "window minimize failed", e);
    }
  }
  ...
  async function toggleMaximizeWindow() {
    try {
      ...
      dbgWarn("layout", "window maximize toggle failed", e);
    }
  }
  ...
  async function closeWindow() {
    try {
      ...
      dbgWarn("layout", "window close failed", e);
    }
  }
  ...
  onMount(() => {
    ...
    getTauriWindow()
      ...
      .catch((e) => dbgWarn("layout", "window listeners unavailable", e));
    return () => {
          ...
          <nav class="flex flex-1 flex-col items-center gap-1 py-2">
            {#each navItems as item}
              ...
              >
                ...
                {#if isActive}
                  <span class="absolute left-0 top-1.5 h-5 w-[3px] rounded-r-full bg-primary"
                  ></span>
                {/if}
